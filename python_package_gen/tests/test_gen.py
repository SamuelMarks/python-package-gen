...
class PackageGenTest(TestCase):
    ...
    default_data_files = sorted(
        listfiles(partial(path.join, templates_pkg_join("config", "_data")))
    )
    ...
    valid_mocks = (
        Namespace(
            ...
            output_directory=path.join(
                gettempdir(), datetime.now().isoformat()
            ).replace(":", "_"),
            single_file=False,
            tests=True,
            version="0.0.1",
    ...
    def test_valid_mocks(self):
        def run_cli_args(cli_args):
            ...
            self.assertEqual(
                ("__init__.py",), tuple(sorted(listfiles(module_name_dir)))
            )
    ...
    def test_invalid_mocks(self):
        def run_cli_args(cli_args):
            ...
            self.assertRaises(
                ArgumentError, lambda: Scaffold(dict(cli_args._get_kwargs()))
            )
