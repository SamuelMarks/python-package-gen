...
from python_package_gen.utils import (
    it_consumes,
    templates_pkg_join,
    listfiles,
    to_module_name,
...
class Scaffold(object):
    def __init__(self, cmd_args):
        ...
        for arg in ("name", "output_directory"):
            if not cmd_args[arg]:
                raise ArgumentError(
                    namedtuple("Argument", "dest option_strings metavar")(
                        "{!r}".format(arg), None, None
                    ),
                    "required",
                )
        ...
        if self.pkg_rename:
            ...
            if not self.flatten:
                cmd_args["output_directory"] = path.join(
                    cmd_args["output_directory"], cmd_args["package_name"]
                )
            print(
                "Module name invalid. {!r} renamed to: {!r}".format(
                    prev_output_directory, cmd_args["output_directory"]
                )
            )
    ...
    def gen_tree(self):
        ...
        return (
            Tree(
                pkg_dir=templates_pkg_join("config"),
                fnames=listdir(templates_pkg_join("config")),
                to_subdir=tuple(),
            ),
            Tree(
                pkg_dir=templates_pkg_join("config", "_data"),
                fnames=listdir(templates_pkg_join("config", "_data")),
                to_subdir=(
                    self.package_name,
                    "_data",
                ),
            ...
            Tree(
                pkg_dir=templates_pkg_join(),
                fnames=listfiles(templates_pkg_join),
                to_subdir=(self.package_name,),
            ),
        )
    ...
    def create_files_and_folders(self):
        ...
        for tree in self.trees:
            for fname in tree.fnames:
                ...
                if not to_loc.endswith("setup.py"):
                    print(
                        "Writing to: {relative_filepath!r}...".format(
                            relative_filepath=to_loc
                        )
                    )
        ...
        print(
            "Writing to: {relative_filepath!r}...".format(
                relative_filepath=new_setup_py
            )
        )
    ...
    def to_single_file(self):
        ...
        assert len(listdir(self.output_directory)) == 1, (
            "Can't convert to single file project with"
        ...
        move(
            path.join(path.dirname(self.output_directory), "__init__.py"),
        ...
        new_setup_py = "".join(
            ...
            for line in fileinput(
                path.join(path.dirname(self.output_directory), "setup.py")
            )
        ...
        with open(
            path.join(path.dirname(self.output_directory), "setup.py"), "wt"
        ) as f:
